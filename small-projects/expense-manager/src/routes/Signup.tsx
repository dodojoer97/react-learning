...
const Signup: FC = () => {
	...
	// Input type togglers
	const { type: password1InputType, toggleInputType: togglePassword1Type } =
		useToggleInputType()
	const { type: password2InputType, toggleInputType: togglePassword2Type } =
		useToggleInputType()

	// Check if the passowrds are equal
	const arePasswordsEqual: boolean = checkValuesEqual(
	...
	// Handle submit
	const { handleSubmit, isSubmitted, setIsSubmitted } = useFormSubmission(
		async () => {
			if (hasErrors) return
